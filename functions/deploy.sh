#!/bin/bash
cd $(dirname $0)
gcloud functions deploy yosatweets_insert_tweets --entry-point main_insert_tweets --runtime python37 --trigger-http --memory 2048MB --timeout 500s --ingress-settings internal-only --allow-unauthenticated
